...
const { isFieldDirty, handleSubmit, resetForm, setFieldValue, setErrors, values } = useForm<
...
const onSubmit = handleSubmit(async (form) => {
  if (file.value) {
    ...
    setErrors({ thumbnail: 'กรุณาอัปโหลดรูปภาพ' })
    return
  }
  ...
  try {
    const res = await createVoucher({
      ...
      expiredAt: dayjs(form.expiredAt).format('YYYY-MM-DD 00:00:00'),
      status: form.status!,
    ...
    console.error(error)
    toast({
      description: 'ไม่สามารถสร้างคูปองได้',
      duration: 3000,
      variant: 'destructive'
    })
  }
                ...
                <div class="grid gap-3">
                  ...
                  >
                    <FormItem v-auto-animate>
                      ...
                      <FormControl>
                        <Input type="text" placeholder="กรอกโค้ด" v-bind="componentField" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  </FormField>
                    ...
                    <FormControl>
                      <Calendar
                        ...
                        @update:model-value="
                          (v) => {
                            if (v) {
                              setFieldValue('beginAt', v.toDate(getLocalTimeZone()))
                            } else {
                    ...
                    <FormControl>
                      <Calendar
                        ...
                        @update:model-value="
                          (v) => {
                            if (v) {
                              setFieldValue('expiredAt', v.toDate(getLocalTimeZone()))
                            } else {
