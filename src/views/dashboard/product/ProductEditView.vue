...
import { queryProduct, updateProduct } from '@/api/product'
...
interface Image extends UpdateProductImageRequest {
  file: File
}
interface Option extends UpdateProductOptionRequest {
  key: number
}
...
const loadProduct = async () => {
  try {
    ...
    if (res.status === 200 && res.data) {
      ...
      setFieldValue('category', String(product.category.id) as any)
      options.value = product.options.map((option, index) => ({
        key: index + 1,
        id: option.id,
        name: option.name,
...
const onSubmit = handleSubmit(async (form) => {
  // check trash images and delete
  if (trashImages.value.length > 0) {
    try {
      const res = await Promise.all(trashImages.value.map((url) => deleteFile(url)))
...
const handleUploadFile = async (file: File) => {
  try {
    ...
    if (res.status === 200 && res.data) {
      const maxSort = images.value.reduce(
        (max, image) => (image.sort! > max ? image.sort! : max),
        0
      )
...
const handleTrashImage = (url: string) => {
  trashImages.value.push(url)
  images.value = images.value.filter((image) => !trashImages.value.includes(image.url!))
  setFieldValue('images', images.value)
}
...
onMounted(async () => {
  await Promise.all([loadCategoryNames(), loadProduct()])
})
                      ...
                      <template v-for="(image, index) in images" :key="index">
                        <div class="flex flex-col items-center gap-2">
                                ...
                                <Tooltip>
                                  <TooltipTrigger>
                                    <button
                                      ...
                                      @click.prevent="handleTrashImage(image.url!)"
                                    >
                                      <X class="h-6 w-6" />
                                    </button>
                                  </TooltipTrigger>
                          ...
                          <Select
                            ...
                            :default-value="
                              String(
                                options.find((option) => option.image?.url === image.url)?.key
                              ) ?? undefined
                            "
                            @update:model-value="
